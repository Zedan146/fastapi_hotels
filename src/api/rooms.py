...
async def put_room(room_data: RoomAddRequest, hotel_id: int, room_id: int, db: DBDep):
    ...
    await db.room_facilities.edit_room_with_facilities(room_id, facilities_ids=room_data.facilities_ids)
    await db.session_commit()
...
async def patch_room(hotel_id: int, room_id: int, db: DBDep, room_data: RoomPatchRequest):
    ...
    if "facilities_ids" in _room_data_dict:
        await db.room_facilities.edit_room_with_facilities(room_id, facilities_ids=_room_data_dict["facilities_ids"])
    await db.session_commit()
